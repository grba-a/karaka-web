...
export default function Proof() {
  ...
  return (
    <section ref={root} className="zone-light relative pb-24 pt-4 md:pb-32">
      <div className="shell mx-auto max-w-[1500px]">
        ...
        <ul className="mt-12 grid grid-cols-2 gap-3 md:mt-16 md:grid-cols-4 md:gap-4">
          {proof.gallery.map((g, i) => (
            <li
              key={g.img}
              data-fade
              className={`relative aspect-[3/4] overflow-hidden ${i % 2 ? 'md:translate-y-6' : ''}`}
            >
              <Image
                src={`/img/${g.img}.webp`}
                alt={g.alt}
        ...
        {/* ------------------------------------------------------ citati */}
        <ul className="mt-16 grid gap-x-12 gap-y-9 md:mt-24 md:grid-cols-3">
          {proof.quotes.map((q) => (
